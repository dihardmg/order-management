...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "orderdb" <<-EOSQL
    ...
    INSERT INTO orders (customer_id, order_date, status, total_amount) VALUES
        (1, CURRENT_TIMESTAMP - INTERVAL '1 day', 'DELIVERED', 150.00),
        (2, CURRENT_TIMESTAMP - INTERVAL '2 days', 'PROCESSING', 200.50),
        (3, CURRENT_TIMESTAMP, 'PENDING', 75.25),
        (4, CURRENT_TIMESTAMP - INTERVAL '3 days', 'DELIVERED', 450.00),
        (5, CURRENT_TIMESTAMP - INTERVAL '5 hours', 'SHIPPED', 125.50);
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "productdb" <<-EOSQL
    ...
    INSERT INTO products (name, description, price, stock, category_id, sku, is_active) VALUES
        ('Laptop Gaming ASUS ROG', 'Laptop gaming Intel Core i7, RAM 16GB, SSD 512GB', 15.999.000, 25, 1, 'LAPTOP-ASUS-001', true),
        ('iPhone 15 Pro Max', 'Smartphone Apple 256GB, Titanium Blue', 24.999.000, 15, 1, 'IPHONE-15-PM-256', true),
        ('Kaos Polos Cotton', 'Kaos katun premium 100% cotton, tersedia berbagai warna', 150.000, 500, 2, 'KAOS-POLos-001', true),
        ('Kemeja Batik Pria', 'Kemeja batik premium, bahan katun primisima', 350.000, 100, 2, 'BATIK-KEMEJA-001', true),
        ('Buku Panduan Java 21', 'Buku pemrograman Java 21 untuk pemula dan mahir', 450.000, 75, 3, 'BUKU-JAVA-21', true),
        ('Vitamin C 1000mg', 'Suplemen vitamin C untuk daya tahan tubuh', 125.000, 200, 4, 'VITAMIN-C-1000', true),
        ('Kopi Arabika Premium', 'Kopi arabika specialty 250gr, roasted to order', 175.000, 150, 5, 'KOPI-ARABIKA-250', true);
EOSQL
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "customerdb" <<-EOSQL
    ...
    -- Insert additional addresses for some customers (multiple addresses scenario)
    INSERT INTO customer_addresses (customer_id, address, city, postal_code, is_default) VALUES
